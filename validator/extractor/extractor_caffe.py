...
class CaffeExtractor:
    ...
    def __init__(self, net, net_pt, model, out_path):
        self.net = net
        self.net_pt = net_pt
        self.model = model
    ...
    def extract_model(self):
        filename = self.outpath + "model_weights.model"
    ...
    def extract_weights(self):
        for name, blobs in self.net.params.iteritems():
            for index in range(len(blobs)):
                ...
                filename = self._create_filename(name, "weights_", index)
    ...
    def extract_outputs(self):
        ...
        for name, blobs in self.net.blobs.iteritems():
            ...
            filename = self._create_filename(name, "outs_", 0)
    ...
    def _create_filename(self, name, filetype, index):
        filename = str(name.replace("/", "_"))
        filename = self.outpath + filetype + filename + "_" + str(index) + ".data"
        return filename
